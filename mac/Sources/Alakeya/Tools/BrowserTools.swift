...
struct BrowserTools: ToolHandler {
    ...
    private static let schemas: [[String: Any]] = [
        fn(
            "browser_open",
            "Открыть URL во встроенном браузере Алакеи только когда пользователь явно просит открыть/перейти/управлять страницей. НЕ используй для обычного интернет-поиска, вопросов, фактов, новостей, рейтингов или анализа сайта — для этого используй search_internet/browser_agent_search без UI-браузера.",
            ["url": ["type": "string", "description": "URL для открытия во встроенном браузере. Не поисковый запрос."]],
            ["url"]
        ),
        fn(
            "browser_read_page",
